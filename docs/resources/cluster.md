...
| Resize pool `compute_vcpu` | ✅ Supported | When `cache_gb` is omitted, the API auto-scales disk proportionally; pin `cache_gb` to keep a specific value (the provider sends a follow-up PATCH after vcpu) |
| Resize pool `cache_gb` | ✅ Supported | CPU and cache cannot be changed in the same API call — the provider issues sequential PATCHes |
| Add `subscription` pool to existing `on_demand` cluster | ⚠️ API behavior currently rejects this in sandbox; provider issues split PATCH (vcpu first, then cache_gb) when accepted |
| Add `on_demand` pool to existing `subscription` cluster | ⚠️ Same sandbox rejection as above |
...
~> **Note on `cache_gb` and `compute_vcpu`:**
> - The API rule **"computeVcpu and cacheGb cannot be updated at the same time"** is enforced server-side. The provider issues sequential PATCHes (vcpu first, then cache_gb) so a single Terraform apply that changes both attributes works.
> - When `cache_gb` is **omitted** from the pool block, the API auto-scales disk proportionally to `compute_vcpu`. To pin a specific `cache_gb`, set it explicitly — the provider sends a follow-up cache PATCH after a vcpu change to enforce your value.
> - The API enforces a **6-hour cooldown on disk resize**. Re-pinning `cache_gb` immediately after a vcpu change can return `409 "No more than 6 hours since the last"` — wait or accept the auto-scaled value.
> - **Minimums** (validated client-side at `terraform plan`): `compute_vcpu >= 4`, `cache_gb >= 100`. Valid `compute_vcpu` values are 4, 8, 16, and multiples of 16.
...
- `compute_vcpu` (Number) vCPU capacity of the subscription pool. Minimum `4`. Valid values are `4`, `8`, `16`, and multiples of `16`.
...
- `cache_gb` (Number) Cache capacity in GB. Minimum `100`. **Optional** — when omitted the API auto-scales disk proportionally to `compute_vcpu`. Set explicitly to pin a value.
...
- `compute_vcpu` (Number) vCPU capacity of the on-demand pool. Minimum `4`. Valid values are `4`, `8`, `16`, and multiples of `16`.
